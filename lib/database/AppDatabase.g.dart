    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `History` (`id` TEXT NOT NULL, `articleInfo` TEXT NOT NULL, `idName` TEXT NOT NULL, `createTime` INTEGER NOT NULL, PRIMARY KEY (`id`))');
...
class _$HistoryDao extends HistoryDao {
  ...
  @override
  Future<List<History>> queryHistory() async {
    return _queryAdapter.queryList('SELECT * from History order by createTime',
        mapper: (Map<String, Object?> row) => History(row['id'] as String,
            row['articleInfo'] as String, row['idName'] as String,
            createTime: row['createTime'] as int?));
  }

  @override
  Future<List<History>> queryHistoryByPage(int offset, int limit) async {
    return _queryAdapter.queryList(
        'SELECT * from History order by createTime desc limit ?2 offset ?1',
        mapper: (Map<String, Object?> row) => History(row['id'] as String,
            row['articleInfo'] as String, row['idName'] as String,
            createTime: row['createTime'] as int?),
